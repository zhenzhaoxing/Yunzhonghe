            ...
            <div class="form" onselectstart="return false;">
                ...
                <div id="o-password">
					...
					<div class="item" id="dphone">
						...
						<a href="javascript:void(0)" class="sendsms" onclick="tosend()" style="color: blue;">获取短信验证码</a>
					</div>
					
					
					
						<div class="item" >
						<span class="label"><b class="ftx04">*</b>请输入验证码</span>

						<div class="fl item-ifo">
							<input type="text" id="yan" maxlength="6" name="yan"
								class="text" tabindex="4"
								autocomplete="off" /> <i class="i-phone"></i> <label
								id="phone_succeed" class="blank"></label> <label
								id="phone_error"></label>
                ...
                <div class="item item-new">
                    ...
                    <div class="fl item-ifo">
                        ...
                        <label for="protocol">我已阅读并同意<a href="#" class="blue" id="protocol">《云中鹤用户注册协议》</a></label>
                        <span class="clr"></span>
...
$(function(){
  ...
  $(".sendsms").click(function() {
             ...
             $.get("sendCode.do?phone="+$("#phone").val(), function(data) {
             	if(data.status==200){
           		   alert("发送成功")
		           }else{
		           alert(data.msg)
	...
	var REGISTER={
		...
		beforeSubmit:function() {
				//检查用户是否已经被占用
				$.ajax({
	            	url : REGISTER.param.surl + "/user/check?username="+$("#regName").val()+"&r=" + Math.random(),
	            	success : function(data) {
	            		if (data.status==200){//等于200说明不存在
	            			//检查手机号是否存在
	            			$.ajax({
	            				url : REGISTER.param.surl + "/user/checknum/"+$("#phone").val()+"/2?r=" + Math.random(),
				            	success : function(data) {
				            		if (data.status==200) {
					            		REGISTER.doSubmit();
				            		} else {
		...
		doSubmit:function() {
			$.post("/user/register?yan="+$("#yan").val(),$("#personRegForm").serialize(), function(data){
				if(data.status == 200){
					alert('用户注册成功，请登录！');
					REGISTER.login();
				} else {
