...
public class ActiveMqTest {
	...
	public void testQueueProducer() throws Exception {
		...
		ConnectionFactory connectionFactory = new ActiveMQConnectionFactory("tcp://192.168.0.122:61616");
	...
	public void testTopicProducer() throws Exception {
		...
		/*TextMessage textMessage = new ActiveMQTextMessage();
		textMessage.setText("hello Activemq");*/
		TextMessage textMessage = session.createTextMessage("topic message this 发布和订阅模式");
		//8、发送消息
		producer.send(textMessage);
		//9、关闭资源
		producer.close();
	...
	@Test
	public void testTopicConsumers() throws Exception {
		...
		Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE);
		//创建一个Destination对象。topic对象
		Topic topic = session.createTopic("springitemAddTopic");
		//使用Session对象创建一个消费者对象。
		MessageConsumer consumer = session.createConsumer(topic);
		//接收消息
		consumer.setMessageListener(new MessageListener() {
			...
			public void onMessage(Message message) {
				...
				try {
					text = textMessage.getText();
					TbItemParamChild pojo = JsonUtils.jsonToPojo(text, TbItemParamChild.class);
